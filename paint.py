...
def iniciarDesenho(event):
    ...
    if forma_atual == "Rabisco":
        ...
        figura_preview = ("Rabisco", [(x_ini, y_ini)], 0, 0, 0, cor_borda, cor_preenchimento) 
...
def moverDesenho(event):
    ...
    # LÓGICA RABISCO:
    if forma_atual == "Rabisco":
        if figura_preview and figura_preview[0] == "Rabisco":
            pontos  = figura_preview[1]
            pontos.append((x_atual, y_atual))
        else:
            pontos = [(x_ini, y_ini), (x_atual, y_atual)]

        figura_preview = figura_preview = ("Rabisco", pontos, 0, 0, 0, cor_borda, cor_preenchimento)
...
def desenhar_tudo():
    ...
    for fig, x1, y1, x2, y2, cborda, cpreencher in figuras:
        if fig == "Retângulo":
            ...
            if len(x1) > 1:
                canvas.create_line(x1, fill=cborda)
    ...
    if figura_preview:
        ...
        if fig == "Retângulo":
            ...
            if len(x1) > 1:
                canvas.create_line(x1, fill = cborda, dash=(4, 2))
